...
lines(as.Date(data[(reg_end+1):nrow(data),1]),rep(average,nrow(data) - reg_end))
lines(as.Date(data[(reg_end+1):nrow(data),1]),rep(average - 0.5*std,nrow(data) - reg_end),col="red")
lines(as.Date(data[(reg_end+1):nrow(data),1]),rep(average + 0.5*std,nrow(data) - reg_end),col="red")
...
ggplot()+
  geom_line(data = data[reg_end:nrow(data),], aes(x=as.Date(data[reg_end:nrow(data),1]), y = money))+
  xlab("Time")+
  ylab("Value")    
