...
flag <- vector()
flag <- append(flag,list(c(data[reg_start,1],"平仓")))

for(i in (reg_start + 1):reg_end){
  ...
  if (flag[[length(flag)]][2] == "平仓" && data[i,2]-coff*data[i,3] > average + 0.5*std && data[i,2]-coff*data[i,3] < data[i-1,2]-coff*data[i-1,3]){
  ...
  if (flag[[length(flag)]][2] == "做空" && data[i,2]-coff*data[i,3] - average  < 0*std){
    flag <- append(flag,list(c(data[i,1],"平仓")))
    volume <- append(volume,0)
  }
  
  if (flag[[length(flag)]][2] == "做多" && data[i,2]-coff*data[i,3] - average  > 0*std){
    flag <- append(flag,list(c(data[i,1],"平仓")))
    volume <- append(volume,0)
  }
  
  if (flag[[length(flag)]][2] == "做空" && data[i,2]-coff*data[i,3] - average > 3*std && which(data[,1] == flag[[length(flag)]][1]) != i){
    flag <- append(flag,list(c(data[i,1],"平仓")))
    break
  }
  
  if (flag[[length(flag)]][2] == "做多" && data[i,2]-coff*data[i,3] - average < -3*std && which(data[,1] == flag[[length(flag)]][1]) != i){
    flag <- append(flag,list(c(data[i,1],"平仓")))
    break
  }
}

plot(as.Date(data[(reg_start + 1):reg_end,1]), data[(reg_start + 1):reg_end,2]-coff*data[(reg_start + 1):reg_end,3],type = "l",xlab="Time",ylab="value of portfolio")
lines(as.Date(data[(reg_start + 1):reg_end,1]),rep(average,reg_end-reg_start))
lines(as.Date(data[(reg_start + 1):reg_end,1]),rep(average-0.5*std,reg_end-reg_start),col="red")
lines(as.Date(data[(reg_start + 1):reg_end,1]),rep(average+0.5*std,reg_end-reg_start),col="red")
